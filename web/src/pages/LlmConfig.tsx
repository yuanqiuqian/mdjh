...
export default function LlmConfig() {
  ...
  return (
    <AppFrame
      title="模型配置"
      subtitle="开局前必须填写 endpoint / model_id / api_key，并测试通过。配置可选择仅保存在内存中，或记住此设备。"
    >
      <div className="grid gap-4 lg:grid-cols-[1.1fr_0.9fr]">
        <SectionCard eyebrow="OpenAI 兼容" title="接入信息">
          <form onSubmit={submit} className="grid gap-4">
            ...
            <label className="flex items-center gap-3 rounded-[18px] border border-white/10 bg-white/5 px-4 py-3 text-sm text-stone-200">
              ...
              />
              <span>记住此设备（保存到本地 IndexedDB，可在卷册导出时选择是否包含 key）</span>
            </label>
        ...
        <div className="grid gap-4">
          <SectionCard eyebrow="当前状态" title="验证结果">
            <div className="grid gap-2 text-sm text-stone-300">
              <p className="text-sm text-stone-100">
                {validation.status === "success" ? "已通过" : validation.status === "error" ? "未通过" : "尚未测试"}
              </p>
              <p className="text-xs leading-5 text-stone-400">{validation.message}</p>
              {typeof validation.latencyMs === "number" ? (
                <p className="text-xs text-stone-500">耗时：{validation.latencyMs}ms</p>
              ) : null}
            </div>
          </SectionCard>

          <SectionCard eyebrow="安全提示" title="不要把 key 暴露出去">
            <div className="space-y-3 text-xs leading-6 text-stone-400">
